...
v2, Tue Sep 15 2026 (thesis-inspector 09-15.3): adds enterpriseToEbitda → evebitda and assetProfile.industry → industry.
  The board now reads ① fwd P/E · ② PEG · ③ P/S · ④ EV/EBITDA against Damodaran industry medians carried IN THE BOARD (peer set mapped
  from the FMP/Yahoo industry name; override per name on the board). secpe (the old sector-median seed) is still written for older builds.
...
What Yahoo carries → board field
  ...
  freeCashflow ÷ marketCap → fcfy (%) · debtToEquity → de (Yahoo reports it in %, divided by 100 here) · enterpriseToEbitda → evebitda (v2)
  assetProfile.industry → industry (v2; the board maps it to a Damodaran peer set when the live FMP industry is unavailable)
...
def one(sym, today, s):
    ...
    sector = ap.get("sector") or ""; industry = ap.get("industry") or ""
    ...
    d = {
        ...
        "evebitda": r(evebitda) if (evebitda is not None and evebitda > 0) else None,
        "sector": sector, "industry": industry, "secpe": SECTOR_FPE.get(sector),
...
def main():
    ...
    for s in names:
        try:
            out[s] = one(s, today, ses); print(f"{s:6s} pe {out[s]['pe']} fpe {out[s]['fpe']} peg {out[s]['peg']} ps {out[s]['ps']} ev/ebitda {out[s]['evebitda']} tmed {out[s]['tmed']} n {out[s]['tn']} rev {out[s]['epsrev'] or '-'} margin {out[s]['margin'] or '-'} {out[s]['sector']} / {out[s]['industry']}")
