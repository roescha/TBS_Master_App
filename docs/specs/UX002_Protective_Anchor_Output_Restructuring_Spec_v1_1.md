...
**Spec Version:** v1.1
**Status:** CLOSED (S173) — Phase 2 implemented (branch `ux002-protective-anchor-restructure` MERGED to `master`) + Phase 3 live PASSED + Phase 4 6-Doc DIA synced; Finding A (§4.1 example guard) corrected v1.0 → v1.1
...
if _hf_timeframe == "DAILY" and _daily_atr_val is not None and _daily_atr_val > 0:
...
Guard: `_hf_timeframe == "DAILY"` (the site-local Profile-A indicator at the `higher_frame` assembly block — `_p_code` is not in scope here; **Finding A correction, v1.1**) **and** `Daily_ATR` present **and** `> 0` (matching the `output.py:2597` / `:3410` guards and `data.py:684` B/C `0.0` initialisation, so Profile B/C are suppressed). `Daily_ATR` is written upstream at `output.py:2603` (before the transform layer reads `flat_metrics`), so it is available at assembly time (pipeline-order confirmed, §11 ITEM 4).
...
| v1.1 | S173 (Phase 4) | **Finding A correction.** §4.1 example guard `_p_code == "A"` → site-local `_hf_timeframe == "DAILY"` (the `higher_frame` assembly block does not have `_p_code` in scope; the shipped `master` code uses `_hf_timeframe == "DAILY"` + `_daily_atr_val > 0`); §4.1 prose guard description updated to match shipped (added `> 0` guard rationale + `data.py:684` B/C `0.0` suppression). Zero contract change — the relocated field shape, scope (Profile A only), and behaviour are unchanged; this corrects the illustrative example to the as-shipped indicator. Status → CLOSED (Phase 2 MERGED + Phase 3 live PASSED + Phase 4 6-Doc DIA synced: Doc 2 v8.67, Doc 8 v8.7.67, Doc 7 v8.5.57, EEM v2.42 verify-only, README v8.6.37, PEO v9.29). |
