...
class TestGateContextRegimeProfileA:
    ...
    def test_nominal_fail_no_golden_cross(self, metrics):
        ...
        assert result.reason == "CONTEXT REGIME FAILED"
        assert result.legacy_diagnostic is not None
        assert "Daily Golden Cross absent" in result.legacy_diagnostic
    ...
    def test_nominal_fail_price_below_sma200(self, metrics):
        ...
        assert isinstance(result, GateResult)
        assert result.verdict == "INVALID"
        assert "Price below Daily SMA 200" in result.legacy_diagnostic
...
class TestGateContextRegimeProfileB:
    ...
    def test_nominal_fail_declining(self, metrics):
        ...
        assert result.reason == "CONTEXT REGIME FAILED"
        assert result.legacy_diagnostic is not None
        assert "Weekly SMA 50 declining" in result.legacy_diagnostic
