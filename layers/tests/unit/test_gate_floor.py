...
class TestGateFloorFailure:
    ...
    def test_nominal_pass_no_failure(self):
        """consec_below=0, is_floor_failure=False — gate passes."""
        result = _gate_floor_failure(
            consec_below=0, is_floor_failure=False, p_code="A"
        )
    ...
    def test_not_floor_failure_high_consec(self):
        ...
        result = _gate_floor_failure(
            consec_below=10, is_floor_failure=False, p_code="A"
        )
        assert result is None
...
class TestGateFloorViolation:
    ...
    def test_nominal_pass_above_floor(self):
        """floor_dist=0.3 (above floor) — gate passes."""
        result = _gate_floor_violation(
            floor_dist=0.3, is_violated=False, p_code="B"
        )
        assert result is None

    def test_nominal_fail_below_floor(self):
    ...
    def test_boundary_exactly_at_floor(self):
        ...
        result = _gate_floor_violation(
            floor_dist=0.0, is_violated=False, p_code="B"
        )
        assert result is None
    ...
    def test_boundary_at_threshold(self):
        ...
        result = _gate_floor_violation(
            floor_dist=-0.15, is_violated=False, p_code="B"
        )
        assert result is None
    ...
    def test_variant_already_violated(self):
        ...
        result = _gate_floor_violation(
            floor_dist=-0.6, is_violated=True, p_code="B"
        )
